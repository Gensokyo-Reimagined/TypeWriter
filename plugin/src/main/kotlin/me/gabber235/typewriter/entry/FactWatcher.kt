...
class FactWatcher(
    ...
    fun refreshFact(ref: Ref<ReadableFactEntry>) {
        ...
        val new = Pair(fact.readForPlayersGroup(player).numValue,fact.readForPlayersGroup(player).strValue)
        if (old != new) {
            factWatch[ref] = new
            notifyListeners(ref)
        }
    ...
    fun addListener(
        ...
        for (fact in facts) {
            factWatch.computeIfAbsent(fact) { Pair(
                    fact.get()?.readForPlayersGroup(player)?.numValue ?: 0,
                    fact.get()?.readForPlayersGroup(player)?.strValue ?: "") }
        }
