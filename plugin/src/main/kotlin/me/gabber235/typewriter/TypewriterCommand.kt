        ...
        fun Player.listCachedFacts(source: CommandSender) {
            ...
            for (entry in factEntries) {
                ...
                source.sendMini(
                    ...
                    }\n\n<gray><i>Click to modify'><click:suggest_command:'/tw facts $name set ${entry.name} ${data.numValue} ${data.strValue}'><gray> - </gray><blue>${entry.formattedName}:</blue> ${data.numValue} ${data.strValue} <gray><i>(${
                        formatter.format(
                            data.lastUpdate
                        )
                    })</i></gray>"
